            ...
            <table class="">
                ...
                <?php if(!empty($list)): if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
                            ...
                            <td style="text-align: center;">
								...
								<a onClick="javascript:if(!confirm('此操作会清理价格不在刷单价格区间的未成交刷单订单(不包含用户订单),确认吗？')) return false;" href="<?php echo U('Config/tradeclear?type=2&id='.$vo['id']);?>" class="btn btn-info btn-xs">清理刷单(部分)</a>
								<a onClick="javascript:if(!confirm('此操作会清理所有未成交刷单订单(不包含用户订单),确认吗？')) return  false;" href="<?php echo U('Config/tradeclear?type=1&id='.$vo['id']);?>" class="btn btn-info btn-xs">清理刷单(全部)</a>
                            </td>
