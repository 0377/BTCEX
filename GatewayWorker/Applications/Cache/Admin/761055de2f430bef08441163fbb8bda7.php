...
<div id="main-content">
	...
	<div id="main" class="main">
		<div class="main-title-h">
            ...
            <span class="h1-title"><?php if(empty($data)): ?>添加文章<?php else: ?>编辑文章<?php endif; ?></span>
		</div>
		<div class="tab-wrap">
				...
				<form id="form" action="<?php echo U('Article/edit');?>" method="post" class="form-horizontal" enctype="multipart/form-data">
					<div id="tab" class="tab-pane in tab">
						<div class="form-item cf">
							<table>
								<tr class="controls">
									<td class="item-label">栏目分类 :</td>
									<td>
										<select name="pid" class="form-control input-10x">
											<option value="">---选择栏目---</option>
											<?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$v): $mod = ($i % 2 );++$i;?><option value="<?php echo ($v["id"]); ?>" <?php if(($v["id"]) == $data['pid']): ?>selected<?php endif; ?>><?php echo ($v["title"]); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
										</select>
									</td>
									<td class="item-note"></td>
								</tr>
								<tr class="controls">
									<td class="item-label">文章标题（中文） :</td>
									<td>
								...
								<tr class="controls">
									<td class="item-label">文章标题（英文） :</td>
									<td>
										<input type="text" class="form-control input-10x" name="title_en" value="<?php echo ($data["title_en"]); ?>">
									</td>
									<td class="item-note"></td>
								</tr>
									...
									<td>
										<div id="addpicContainer">
											<?php if(!empty($data["img"])): ?><!--没有图片显示默认图片-->
												<img id="up_img" onclick="getElementById('inputfile').click()" style="cursor:pointer;max-width:100px;" title="点击添加图片" alt="点击添加图片" src="/Upload/wenzhang/<?php echo ($data["img"]); ?>">
												<?php else: ?>
												<!--没有图片显示默认图片-->
								...
								<tr class="controls">
									<td class="item-label">文章内容（中文） :</td>
									<td>
								...
								<tr class="controls">
									<td class="item-label">文章内容（英文） :</td>
									<td>
										<textarea name="content_en" class="form-control input-10x"><?php echo ($data["content_en"]); ?></textarea>
									</td>
									<td class="item-note"></td>
								</tr>
								<tr class="controls">
									<td class="item-label">文章置顶 :</td>
									<td>
										<select name="recommend" class="form-control input-10x">
											<option value="0" <?php if(($data["recommend"]) == "0"): ?>selected<?php endif; ?>>不置顶</option>
											<option value="1" <?php if(($data["recommend"]) == "1"): ?>selected<?php endif; ?>>置顶</option>
										</select>
									</td>
									<td class="item-note"></td>
								</tr>
								
								<?php if(($data['addtime']) == ""): else: ?>
									<tr class="controls">
...
<script type="text/javascript">
	...
	KindEditor.ready(function (K) {
		...
		editor_en = K.create('textarea[name="content_en"]', {
			width: '800px',
			height: '300px',
			allowPreviewEmoticons: false,
			allowImageUpload: true,
		...
		$("#inputfile").change(function () {
			...
			$.ajax({
				...
				success: function (data) {
					if (data) {
						$('#up_img').attr("src", '/Upload/wenzhang/' + $.trim(data));
						$('#img').val(data);
						$('#up_img').show();
					}
				},
