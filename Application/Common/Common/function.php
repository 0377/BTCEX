 ...
 * @return           string
 */
function GetPosStr($cid=0,$id=0,$sign='&nbsp;&gt;&nbsp;',$lang='zh-cn')
{
//    return $lang;
	//设置首页链接
    $urlStr = strtolower(U('/Support'));
	$pos_str = "<li><a href=".$urlStr.">".L('帮助中心')."</a></li>";
	...
	if(!empty($cid))
	{
		...
		if (empty($r['id'])) {
            if($lang == 'zh-cn'){
                return $pos_str.'<li><span>&gt;</span>栏目不存在'."</li>";
            }else{
                return $pos_str.'<li><span>&gt;</span>Column does not exist</li>';
            }
		} else {
			//构成上级栏目字符
			if ($r['pid'] != '0') {
				...
				if ($cvv['pid'] > 0) {
					...
					$pos_str .= "<li><span>&gt;</span><a href=".U('Support/index/categories/cid/'.$cvv['pid']).">";
					if($lang == 'zh-cn'){
                        $pos_str .= $r['title']."</a></li>";
                    }else{
                        $pos_str .= $r['title_en']."</a></li>";
                    }
				}
			}
			...
			if (isset($r) && is_array($r)) {
				if (!empty($id)) {
                    $pos_str = $pos_str."<li><span>&gt;</span><a href=".U('Support/index/sections/cid/'.$r['pid'].'/id/'.$r['id']).">";
                    if($lang == 'zh-cn'){
                        $pos_str .= $r['title']."</a></li><li><span>&gt;</span>正文</li>";
                    }else{
                        $pos_str .= $r['title_en']."</a></li><li><span>&gt;</span>Content</li>";
                    }
                    return $pos_str;
				} else {
                    $pos_str = strtolower($pos_str)."<li><span>&gt;</span>";
                    if($lang == 'zh-cn'){
                        $pos_str .= $r['title']."</li>";
                    }else{
                        $pos_str .= $r['title_en']."</li>";
                    }
                    return $pos_str;
				}
			} else {
...
{
    //DATABASE_PATH定义备份文件路径
	$file_path = DATABASE_PATH . '/core.json';

	//检查判断文件是否存在
	if (file_exists($file_path)) {
	    //若文件存在那么匹配正则 然后进行替换 返回替换完成后的json字符串
...
function Astccommon($host)
{
    return new \Common\Ext\AstcCommon($host);
}
/*
* 资金变更日志（前后台都含有）
...
function EasyBitcoin($username, $password, $ip, $port, $url = null)
{
    return new \Common\Ext\EasyBitcoin($username, $password, $ip, $port, $url);
}
...
function AstcClient( $ip, $port)
{
    return new \Common\Ext\AstcCommon($ip, $port);
}
 ...
 * IP限制
 */
function ip_astrict($timeBlock = 60,$num){
    $ip = get_client_ip(0,true);
    $userReg = M('user_reg_log');
    $where = ['ip'=>$ip];
    $reglog = $userReg->where($where)->field('num,state')->find();
    if(!$reglog){
        $userRegValue = [
            'ip' => $ip,
            'addr' => get_city_ip()
        ];
        $userReg->add($userRegValue);
        return true;
    }
    $userReg->where($where)->setInc('num');
    if($reglog['state'] == 1){
        return true;
    }
    if($reglog['num'] >= $num){
 ...
 * IP次数限制
 */
function ip_astrict_num($timeBlock = 60,$num=100000){
    $ip = get_client_ip(0,true);
    if($ip = '127.0.0.1'){
        return true;
    }
    $userReg = M('user_reg_ip_log');
    $time = date('Y-m-d H:i:s',(strtotime()-$timeBlock));
    $where = [
        'ip'=>$ip,
        'create_time'=>['egt',$time]
    ];
    $ipNum = $userReg->where($where)->count();
    if($ipNum>=$num){
        return false;
    }
    $data = [
...
// 短信接口
function sendsmsint($moible,$content,$sign,$intnum)
{
	//请使用您自己的开发者KEY
	$config = M('Config')->where(array('id' => 1))->find();
...
//	    exit;
        // 国内短信
        $result = $sendsms->quickPost('http://api.1cloudsp.com/api/v2/single_send', array(
            'accesskey' => $accesskey,
            'secret' => $accessScrect,
            'templateId' => '30666',
            'mobile' => $moible,
...
// 管理员密码加密
function admin_md($pwd){
    $salt = ADMIN_SALT;
    $admin_pwd = md5(md5($pwd).$salt);
    return $admin_pwd;
}

// 用户密码加密
function user_md($pwd){
    $salt = USER_SALT;
    $admin_pwd = md5(md5($pwd).$salt);
    return $admin_pwd;
}
...
// 查询user_coin表
function queryUserCoin($userid,$coin_name,$coin_addr){
    if($userid){
        $where['userid'] = $userid;
    }
    if($coin_name){
    ...
    if($coin_addr){
        $where[$coin_name.'b'] = $coin_addr;
    }
    $result = M('UserCoin')->where($where)->find();
    return $result;
...
// user_coin表操作写入表
function crateRecord($userid,$coinName,$type,$address='',$remark='',$coin_num='0.00',$coin_old='0.00')
{
    $data=array('userid'=>$userid,'coin'=>$coinName,'coin_num'=>$coin_num,'coin_old'=>$coin_old,'type'=>$type,'address'=>$address,'remark'=>$remark);
    $result = M('UserCoinLog')->add($data);
    if($result){
        return $result;
    }else{
        return $result;
