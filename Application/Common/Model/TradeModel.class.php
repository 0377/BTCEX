...
class TradeModel extends \Think\Model
{
	...
	public function chexiao($id = NULL)
	{
        ...
        if(!$uid && $from != 0){
            return ['0','系统错误'];
        }else if($trade['userid']!=$uid && $from != 0){
            return ['0','订单系统错误，请联系管理员'];
        }
		//开启事务
        $mo = M();
        $mo->startTrans();
		...
		//	$mo->execute('lock tables tw_user_coin write  , tw_trade write ,tw_finance write,tw_finance_log write,tw_user write,tw_auth_group_access write,tw_admin write');//处理资金变更日志
			...
			if ($trade['type'] == 1) {
				...
				//获取用户手续费字段 进行判断 0为普通用户 1为做市商 做市商不收取手续费
				if($buyuser['lv']==1){
					$fee_buy=0;
				}


                //订单数量减去订单已成交的数量乘以订单价格除以100 乘以 手续费加100 计算出要退的手续费 然后取小数点后八位
				...
				file_put_contents('./log.txt',$mo->getLastSql().PHP_EOL,FILE_APPEND);
				...
				if ($mun <= round($user_sell[$xnb . 'd'], 8)) {
                    ...
                    //事务回滚
                    $mo->rollback();
					throw new \Think\Exception('撤销失败222222');
				}
