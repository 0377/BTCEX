...
class IndexController extends SupportController
{
	...
	public function index()
	{
		...
		foreach ($classlist as $k => $v) {
			$fenlei = M('ArticleType')->where(array('id'=>$v['pid'],'status'=>1,))->find();
			if($fenlei){
                $articlelist[$k]['id'] = $v['id'];
                $articlelist[$k]['pid'] = $v['pid'];
                $articlelist[$k]['title'] = $v['title'];
                $articlelist[$k]['title_en'] = $v['title_en'];
                $articlelist[$k]['classname'] = $fenlei['title'];
                $articlelist[$k]['classname_en'] = $fenlei['title_en'];
            }
		}
		
		$this->assign('empty','<li style="color:#c1c1c1;">'.L('没有数据').'</li>');
		$this->assign('classlist', $articlelist);
