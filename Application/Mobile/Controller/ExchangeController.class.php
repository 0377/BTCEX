...
class ExchangeController extends MobileController
{
	...
	public function upTrade($price, $num, $otype)
	{
		...
		if ($num < $configs['mycz_min']) {
			$this->error(L('每次提现金额不能小于') . $configs['mycz_min'] . L('元！'));
		}
		if ($configs['mycz_max'] < $num) {
			$this->error(L('每次提现金额不能大于') . $configs['mycz_max'] . L('元！'));
		}
		...
		if ($mycz) {
			$this->success(L('订单创建成功！'),U('Exchange/payinfo',array('oid' => $mycz,'aid' => $data_agent['id'])) );
		} else {
			$this->error(L('订单创建失败！'));
		}
	}
...
/*			if ($v['name'] == $name) {
	...
	public function payinfo($oid,$aid)
	{
		// 过滤非法字符----------------S
		if (checkstr($oid) || checkstr($aid)) {
			$this->error('您输入的信息有误！');
		}
		// 过滤非法字符----------------E
		...
		$data['order'] = M('exchange_order')->where(array('id' => $oid))->find();
