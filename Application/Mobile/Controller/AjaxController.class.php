...
class AjaxController extends MobileController
{
	...
	{
		parent::_initialize();
		$allow_action=array("upImg","getJsonMenu","allfinance","allsum","allcoin","trends","getJsonTop","getJsonTop2","getTradelog","getDepth","getEntrustAndUsercoin","getChat","upChat","upcomment","subcomment","getJsonMobile","top_coin_menu","allcoin_a");
...
/*				foreach (C("market") as $k => $v) {
	...
	public function getEntrustAndUsercoin($market = NULL, $ajax = 'json')
	{
		...
		$userCoin = M('UserCoin')->where(array('userid' => userid()))->find();
		if ($userCoin) {
			$xnb = explode('_', $market)[0]; // hrc
			$rmb = explode('_', $market)[1]; // usdt
