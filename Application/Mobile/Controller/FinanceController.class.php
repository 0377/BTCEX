...
//				$mo->execute('unlock tables');
		...
		if ($Coins['type'] == 'qbb') {
			...
			if ($mo->table('tw_user_coin')->where(array($qbdz => $addr))->find()) {//站内地址互转
				...
				$dj_address = $Coins['dj_zj'];
				$dj_port = $Coins['dj_dk'];
				if($coin=='eth' || $coin=='zil' || $coin=='eos' ||$coin=='trx'||$coin=='blz'||$coin=='fsn'||$coin=='qsp'||$coin=='lrc'||$coin=='red'||$coin=='noc'||$coin=='iost'|$coin=='fff'){
					$auto_status =0;
					/*
				            $EthClient = EthCommon($dj_address, $dj_port);
						...
						if ($auto_status) {
							...
							// $this->success(L('转出申请成功,请等待审核！'));
								...
								$EthClient = EthCommon($dj_address, $dj_port);
							            ...
							            $sendrs = $EthClient->eth_sendTransactionraw($dj_username,$constadd,$dj_password,$dataraw);//转出账户,合约地址,转出账户解锁密码,data值
