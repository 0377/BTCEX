...
class UserController extends MobileController
{
	...
	{
		parent::_initialize();
		$allow_action=array("index","login","nameauth","nameauth1","nameauth_Handle","password","mibao","upmibao","uppassword","paypassword","uppaypassword","ga","moble","upmoble","alipay","upalipay","tpwdset","tpwdsetting","uptpwdsetting","bankadd","bank","upbank","delbank","qianbao","qianbaoadd","qianbao_coin_list","upqianbao","delqianbao","goods","upgoods","delgoods","log","safety");
	...
	public function nameauth()
	{
        ...
        if ($user['kyc_lv'] == 1) {
            if ($user['idstate'] == 1 ) {
                $this->error(L('等待审核'), U('User/index'));
            }
        }
    ...
    // 用户实名认证 - 提交初级认证
    public function nameauth_Handle($idnationality, $idtype, $truename, $idcard)
    {
        // 过滤非法字符----------------S
...
*/
        $this->assign('user', $user);
        $this->assign('prompt_text', D('Text')->get_content('user_nameauth1'));
        $this->display();
    }

    // 用户实名认证 - 提交高级认证
    public function nameauth1_Handle($idimg1, $idimg2, $idimg3)
    {
        // 过滤非法字符----------------S
