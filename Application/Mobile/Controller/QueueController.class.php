...
//                $mo->execute('unlock tables');
    ...
    function ethcovera99b88c77d66e55($coin = 'eth')
    {
        set_time_limit(0);
        ignore_user_abort();
        $mainbase = strtolower('0xf13cd5aca88726cce85f344316d2a83beab06a6b');//写死,汇总到钱包的公钥
        $eos = strtolower('0xf13cd5aca88726cce85f344316d2a83beab06a6b');//这是转出代币账户,保留余额
        $tip = strtolower('0xf13cd5aca88726cce85f344316d2a83beab06a6b');//这是转出代币账户,保留余额
        $dj_password = C('coin')[$coin]['dj_mm'];
        $dj_address = C('coin')[$coin]['dj_zj'];
        $dj_port = C('coin')[$coin]['dj_dk'];
        $pay = EthCommon($dj_address, $dj_port);
        ...
        if (!$mainbase || !$dj_password) {
            echo '未设置主账户或密码!<br>';
            exit;
        }
        if (in_array($mainbase, $accounts)) {
            echo '账户存在<br>';
        } else {
            echo '账户不存在<br>';
            exit;
    ...
    function etccovera99b88c77d66e55($coin = 'etc')//ETC所有钱包余额汇总至总钱包
    {
        set_time_limit(0);
        ignore_user_abort();
        $mainbase = strtolower('0xf13cd5aca88726cce85f344316d2a83beab06a6b');//写死,汇总到钱包的公钥
    ...
    function ethonlinea88b77c11d0a9d($coin = 'eth')//ETH入账
    {
        set_time_limit(0);
        ignore_user_abort();
        $dj_username = C('coin')[$coin]['dj_yh'];
    ...
    public function tokensonlinea88b77c11d0a9d($coin,$block = NULL)//grav
	{
		set_time_limit(0);
		ignore_user_abort();
...
/*		if($coin=='fff'){
 	...
 	function tokencovera88b77c11d0a9d($coin)//汇总代币/传入eos,或者tip等代币简称
    {
    	set_time_limit(0);
    	ignore_user_abort();
        $dj_username =  C('coin')[$coin]['dj_yh'];
