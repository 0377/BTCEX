	...
	public function index($market = NULL)
	{
		...
		if (!$topdata) {
			if ($market) {
                ...
                $coin_reg = strtoupper(explode("_", $market)[1]);
                if($coin_reg == 'USDT'){
                    $jiaoyiqu = 0;
                }elseif ($coin_reg == 'ETH'){
                    $jiaoyiqu = 2;
				...
				foreach (C('market') as $k => $v) {
                    ...
                    $str = substr($v['title_ns'], 1,-1);
                    if(stripos($str, $coin_reg)){
                        $array[]['title_ns'] = $str;
                    }
        ...
        $mark  = strtoupper(str_replace('_', '/', $market) );
...
//		$this->assign('market_coin', Anchor_CNY);
    ...
    public function test($market = NULL)
    {
        // 过滤非法字符----------------S
...
//		$this->assign('market_coin', Anchor_CNY);
	...
	/*public function upTrade($paypassword = NULL, $market = NULL, $price, $num, $type)
	{
		...
		// $this->success(L('交易成功！'));
		//jhsoft即时处理交易状态和异常处理
		A('Home/Queue')->checkDapan();//匹配所有订单交易
    ...
    public function upTrade($paypassword = NULL, $market = NULL, $price, $num, $type)
    {
        ...
        try{
            ...
            if ($type == 1) {
              ...
              /*  $ar = $mo-> table('tw_user_coin')->where(['userid'=>$userid])->field($xnb)->find()[$xnb];
                var_dump($mum) ;
                echo $num;
                exit;
                return $ar;*/
                $rs[] = $mo->table('tw_user_coin')->where(array('userid' => userid()))->setDec($xnb, $mum);
                $rs[] = $mo->table('tw_user_coin')->where(array('userid' => userid()))->setInc($xnb . 'd', $num);
                $rs[] = $mo->table('tw_trade')->add(array('userid' => userid(), 'market' => $market, 'price' => $price, 'num' => $num, 'mum' => $mum, 'fee' => $fee, 'type' => 2, 'addtime' => time(), 'status' => 0));
                $fin_user_coin_new = $mo->table('tw_user_coin')->where(array('userid' => userid()))->find();//处理资金变更日志
