...
class FinanceController extends HomeController
{
	...
	{
        if (!session('userId')) {
            $this->error(L('请登录！'),U('Login/index'));
            //redirect('/Login/index.html');
        }
        $uid = session('userId');
        // 用户后台被禁用
        $result = M('User')->where(array('status'=>0,'id'=>$uid))->find();
        if($result){
...
//					$jia = 0;
	...
	public function qrcode($url=NULL){
		...
		//生成二维码图片
		$object = new \QRcode();
		//$url = 'https://'.$_SERVER['HTTP_HOST'].'/Login/register?invit='.$url;//网址或者是文本内容
        $image = $url.".png"; // 二维码文件名
		$urls = 'http://'.$_SERVER['HTTP_HOST'].'/Login/register?invit='.$url;//网址或者是文本内容
		...
		ob_clean();
//		$codeImg = $object->png($url, false, $errorCorrectionLevel, $matrixPointSize, 2);
//        return $codeImg;

        $object->png($urls, $fileRoot, $errorCorrectionLevel, $matrixPointSize,2); // 生成二维码保存在指定路径
//        if(file_exists($fileRoot)){
//            $result = M('user')->where(array('invit'=>$url))->save(array('invit_img'=>$image));
//        }

        return $fileRoot;

       /* $codeImg = $fileRoot;
        $QR = imagecreatefromstring(file_get_contents($codeImg));
       ...
       // Header("Content-type: image/png");
        ...
        imagedestroy($QR);
        return '<img src="qrcode.png" alt="扫描支付">';*/
	...
	/*public function haibao($url=NULL,$type=0)
	{
        ...
        $filename = '/Public/Home/rh_img/haibao_sh.png'; // 保存图片到服务器

        return getbgqrcode($imageDefault,$textDefault,$background,$filename,$config);
    ...
    function createPoster($config=array(),$filename=""){
        ...
        if(!empty($config['image'])){
            ...
            foreach ($config['image'] as $key => $val) {
                ...
                $info = getimagesize($val['url']);  //获取图片大小及相关信息
...
//                return $canvas;
    ...
    public function haibao($url=NULL){
        ...
        $config = array(
            'image'=>array(
                array(
                    ...
                    // 'url'=>'http://'.$_SERVER['HTTP_HOST'].'/Home/Finance/qrcode/url/'.$url, //二维码
                   'url'=>$img, //二维码
            ...
            'background'=>'Public/Home/rh_img/haibao22.png'          //背景图
        );
        $filename = 'Public/Home/rh_img/tupian.png';

//        var_dump($this->createPoster($config,$filename));
       //return  $this->createPoster($config,$filename);

        $result = $this->createPoster($config,$filename);
        return   $result;
    }
...
//			$mo->execute('lock tables  tw_user_coin write  , tw_myzc write  , tw_myzr write , tw_myzc_fee write,tw_finance_log write,tw_user read');
			...
			if ($fee) {
				if ($user_coins = $mo->table('tw_user_coin')->where(array($qbdz => $Coins['zc_user']))->find()) {
			...
			if (!$user_coin[$qbdz]) {
				...
				if ($Coins['type'] == 'qbb') {
					...
					if (!isset($json['version']) || !$json['version']) {
                        ...
                        if ($user_coin[$coin] == $coin.'b'){
                            $qianbao = $user_coin[$coin];
                        } else{
                            /* $url = C(''create_url); */
                            $url = '47.244.52.38:8802/api/createAddr';
                            $AsClient = Astccommon($url);
                            $qianbao= $AsClient->user_createNewAddr();
                            $log = "创建地址接口：".$url.'生成的信息'.$qianbao;
                            paylog($log);
                            if (!$qianbao || preg_match("/[\'.,:;*?~`!@#$%^&+=)(<>{}]|\]|\[|\/|\\\|\"|\|/",$qianbao)) {
                                $qianbao = L('生成钱包地址出错！');
                                $state_coin = 1;
                            }else{
                                $rs = M('UserCoin')->where(array('userid' => userid()))->save(array('astcb' => $qianbao));
                            }
                        }

                    } else {
						
					if (!isset($json['version']) || !$json['version']) { // btc
						//$this->error('钱包链接失败！');
						$qianbao = L('钱包链接失败！');
						$state_coin = 1;
					} else {
...
/*		if (C('coin')[$coin]) {
		...
		$myzc_min = ($Coinx['zc_min'] ? abs($Coinx['zc_min']) : 1); // 单笔提币最小数量
		$myzc_max = ($Coinx['zc_max'] ? abs($Coinx['zc_max']) : 10000000); // 单笔最大提币数量
		...
		$user_coin = M('UserCoin')->where(array('userid' => userid()))->find();
		$user_coin[$coin] = round($user_coin[$coin], 6);
		$user_coin[$coin] = sprintf("%.4f", $user_coin[$coin]); // 用户的各种币的可用
		$user_coin[$coin.'d'] = round($user_coin[$coin.'d'], 6);
		$user_coin[$coin.'d'] = sprintf("%.4f", $user_coin[$coin.'d']);  // 用户的各种币的冻结
...
//				$mo->execute('unlock tables');
		...
		if ($Coins['type'] == 'qbb') {
			$mo = M();
			$mo->startTrans();
			if ($mo->table('tw_user_coin')->where(array($qbdz => $addr))->find()) {//站内地址互转
				$peer = M('UserCoin')->where(array($qbdz => $addr))->find();
				if (!$peer) {
					$this->error(L('转出地址不存在！'));
				}
				if($coin == 'astc' || $coin == 'ASTC'){ // astc
                    $auto_status = 0;
                    try{
                        $mo = M();
                        $mo->startTrans();
...
//                            $mo->execute('unlock tables');
                            session('myzc_verify', null);
                            $mo->commit();
                            $this->success(L('ASTC转账成功,等待审核！'));
                        } else {
                            throw new \Think\Exception(L('转账失败!00001'));
                        }
                    } catch (\Think\Exception $e){
                        //$mo->execute('rollback');
                        $mo->rollback();
//                        $mo->execute('unlock tables');
                        $this->error(L('转账失败00002'));
                    }
                }elseif($coin == 'usdt' || $coin == 'USDT'){ // usdt转出-中心账号转出 交易所账号- myzc表记录 finance_log 记录
                    $auto_status = 0;
                  try{
                        $mo = M();
                        $mo->startTrans();
                        ...
                        $usdt_config = M('Coin')->where(array('name' => $coin))->find();
                        // 后台配置手续费 -- 扣除账户余额
                        if($usdt_config['zc_fee'] > 0){
                            $sum_num = $num + ($num * $usdt_config['zc_fee']);
                            if ($user_coin[$coin] < $sum_num) {
                                $this->error(L('可用余额不足'));
                            }
                            $rs[] =  $mo->table('tw_user_coin')->where(array('userid' => userid()))->setDec($coin, $sum_num);

                            $rs[] = $aid = $mo->table('tw_myzc')->add(array('userid' => userid(), 'username' => $addr, 'coinname' => $coin, 'num' => $num, 'fee' => $usdt_config['zc_fee'], 'mum' => $mum, 'addtime' => time(), 'status' => $auto_status));
                      ...
                      /*  $rs[] = $mo->table('tw_user_coin')->where(array('userid' => userid()))->setDec($coin, $num);
                        ...
                        if (check_arr($rs)) {
                            session('myzc_verify', null);
                            $mo->commit();
                            $this->success(L('USDT转账成功,等待审核！'));
                        } else {
                            throw new \Think\Exception(L('USDT转账失败!00001'));
                        }
                    } catch (\Think\Exception $e){
                        $mo->rollback();

                        $this->error(L('USDT转账失败00002'));
                    }
...
//					$mo->execute('unlock tables');
				...
				$url = C('BBCOIN').'/home/queue/sendotc?accesskey='.C('BBAPIKEY').'&user='.$user_info['username'].'&num='.$num.'&coin='.$coin;
			    ...
			    if($fanhui['code'] == 0) {
					...
					try {
			        	...
			        	$mo->startTrans();
						//$mo->execute('set autocommit=0');
//						$mo->execute('lock tables  tw_user_coin write  , tw_myzc write  ,tw_finance_log write,tw_user read');
						$rs = array();
						$rs[] = $mo->table('tw_user_coin')->where(array('userid' => userid()))->setDec($coin, $num);
						$rs[] = $mo->table('tw_myzc')->add(array('userid' => userid(), 'username' => '转入场外交易', 'coinname' => $coin, 'txid' => '转入场外交易账户:'.$user_info['username'], 'num' => $num, 'fee' => 0, 'mum' => $num, 'addtime' => time(), 'status' => 1));

						// 处理资金变更日志-----------------S
						$user_zj_coin = $mo->table('tw_user_coin')->where(array('userid' => userid()))->find();

						// 转出人记录
						$mo->table('tw_finance_log')->add(array('username' => $user_info['username'], 'adminname' => $user_info['username'], 'addtime' => time(), 'plusminus' => 0, 'amount' => $num, 'optype' => 6, 'position' => 1, 'cointype' => 3, 'old_amount' => $user_coin[$coin], 'new_amount' => $user_zj_coin[$coin], 'userid' => userid(), 'adminid' => userid(),'addip'=>get_client_ip()));
...
//							$mo->execute('unlock tables');
      ...
      //                       			   }
                ...
                if($coin == 'usdt' || $coin == 'USDT'){ // usdt转出-中心账号转出 交易所账号- myzc表记录 finance_log 记录
                    try{
                        $mo = M();
                        $mo->startTrans();
                        $rs = array();
...
//        var_dump($user['invit']);
        ...
        //$url = $this->haibao($user['invit']);
       // $this->assign('urlHaiBao', $url);

        $img = $this->haibao($user['invit']);

		$this->assign('urlHaiBao', $img['message']);
		$this->assign('user', $user);
