...
class AjaxController extends HomeController
{
	...
	public function allcoin_a($id=1,$ajax='json',$lang=NULL)
	{
		$trandata_data = array();
            ...
            if ($marketLogs) {
                foreach (C('market') as $k => $v) {
                    ...
                    if($lang == 'en-us'){
                        ...
                        $_tradeLogs['mum'] =  $_tradeLogs['mum'] * C('usdt');
                    }

                    if ($_tradeLogs) {
                        $volume_24h[$k] = round($_tradeLogs['num'], 4); // 24小时 交易量

//                        var_dump($volume_24h[$k]);
                        $tradeAmount_24h[$k] = round($_tradeLogs['mum'], 4); // 24小时 交易额
                    }
                }
            }
...
/*        if ($marketLogs) {
		...
		if (!$data) {
			...
			$trandata_data['url'] = "";
			$coins = M('coin')->select();
            $market = M('market')->order("sort desc")->select();
            foreach ($market as $key => $val){
                $name = strtolower(explode('_', $val['name'])[0]);
                foreach ($coins as $k => $v){
                    if($name == $v['name']){
                        $market[$key]['xnbimg'] = $v['img'];
                    }
                }
            }
...
//            foreach (C('market') as $k => $v)
			{
				...
				if ($v['jiaoyiqu'] == $id) {
                    if ($v['show_status'] == 1) {
                        ...
                        if ($lang == 'en-us') {
                            $trandata_data['url'][$k][14] = $market;
                        } else {
                            $trandata_data['url'][$k][14] = 'CNY';
                        }

                        //最新成交价
                        if (empty($v['new_price']) || $v['new_price'] == 0.00) {
                            $trandata_data['url'][$k][2] = "--";
                        } else {
                        ...
                        if($lang == 'en-us'){
                            //最新成交价
                            if (empty($v['new_price']) || $v['new_price'] == 0.00) {
                                $trandata_data['url'][$k][16] = 0;
                            } else {
                                $trandata_data['url'][$k][16] = round($v['new_price'], $v['round']);
                            }
                        }else{
                            //最新成交价
                            if (empty($v['new_price']) || $v['new_price'] == 0.00) {
                                ...
                                $trandata_data['url'][$k][16] = round($v['new_price']* C('usdt'), $v['round']);
                            }
                        }
...
//					$trandata_data['url'][$k][7] = isset($volume_24h[$k]) ? $volume_24h[$k] : 0;//round($v['volume'], 4) * 1;
                        if (empty($volume_24h[$k])) {
                            $trandata_data['url'][$k][7] = "--";
                        } else {
                            $trandata_data['url'][$k][7] = isset($volume_24h[$k]) ? $volume_24h[$k] : "--";
                        }
                        ...
                        //最高价
                        if (empty($v['max_price']) || $v['max_price']==0.00) {
                            $trandata_data['url'][$k][11] = "--";
                        } else {
                        ...
                        if($lang == 'en-us'){
                            //最高价
                            if (empty($v['max_price']) || $v['max_price']==0.00) {
                                $trandata_data['url'][$k][17] = 0;
                            } else {
                                $trandata_data['url'][$k][17] = round($v['max_price'], $v['round']);
                            }
                        }else{
                            //最高价
                            if (empty($v['max_price']) || $v['max_price']==0.00) {
                                ...
                                $trandata_data['url'][$k][17] = round($v['max_price']* C('usdt'), $v['round']);
                            }
                        }

                        //最低价
                        if (empty($v['min_price']) || $v['min_price']==0.00) {
                            $trandata_data['url'][$k][12] = "--";
                        } else {
                        ...
                        if($lang == 'en-us'){
                            //最低价
                            if (empty($v['min_price']) || $v['min_price']==0.00) {
                                $trandata_data['url'][$k][18] = 0;
                            } else {
                                $trandata_data['url'][$k][18] = round($v['min_price'], $v['round']);
                            }
                        }else{
                            //最低价
                            if (empty($v['min_price']) || $v['min_price']==0.00) {
                                ...
                                $trandata_data['url'][$k][18] = round($v['min_price']*C('usdt'), $v['round']);
                            }
                        }

                        $trandata_data['url'][$k][13] = '';
                        $trandata_data['url'][$k][15] = $v['status'];
                    }
                }
			}
		}
	...
	public function allfinance($ajax = 'json')
	{
		...
		foreach (C('coin') as $k => $v) {
			...
			if ($v['name'] == 'cnc') {
				...
				if (C('market')[$v['name'] . '_cnc']['new_price']) {
					$jia = C('market')[$v['name'] . '_cnc']['new_price'];
				} else {
					$jia = 1;
				}
...
/*		exit;
	...
	public function c2cPayment($id, $aid, $ajax = 'json')
	{
		...
		if ($data['order']['otype']  == 1 || $data['order']['otype']  == 2) {
			if ($data['order']['status']  == 1) {
                $data['order']['sta'] = 1;
				$data['order']['status'] = L('待支付');
			}elseif ($data['order']['status']==2){
                $data['order']['sta'] = 2;
			    $data['order']['status'] = '已付款';
            }
			else {
