...
class HuafeiController extends HomeController
{
	...
	public function index()
	{
		if (empty($_POST)) {
			...
			$rs[] = $mo->table('tw_user_coin')->where(array('userid' => userid()))->setDec($type, $mum);
			$rs[] = $huafei_id = $mo->table('tw_huafei')->add(array('userid' => userid(), 'mobile' => $mobile, 'num' => $num, 'type' => $type, 'mum' => $mum, 'addtime' => time(), 'status' => 0));

			if (C('huafei_zidong')) {
				if (huafei($mobile, $num, md5($huafei_id))) {
					$rs[] = $mo->table('tw_huafei')->where(array('id' => $huafei_id))->save(array('endtime' => time(), 'status' => 1));
				}
			}
