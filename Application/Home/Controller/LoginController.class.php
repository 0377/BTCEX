...
//			$mo->execute('unlock tables');
	...
	{
        //IP限制
        $ipState = ip_astrict(60,20);
        if(!$ipState){
            $this->error(L('账号异常，请联系我们!'));
        }
		//$this->error("注册系统升级，暂停注册！20:30开放注册",U('Login/index'));
		// 过滤非法字符----------------S
...
//		$mo->execute('lock tables tw_user write , tw_user_coin write , tw_invit write ');
		$rs = array();
		$rs[] = $mo->table('tw_user')->add(array('username' => $mobile, 'mobile'=>$mobile, 'mobiletime'=>time(), 'password' => user_md($password), 'invit' => $tradeno, 'tpwdsetting' => 1, 'invit_1' => $invit_1, 'invit_2' => $invit_2, 'invit_3' => $invit_3, 'addip' => get_client_ip(), 'addr' => get_city_ip(), 'addtime' => time(), 'status' => 1 , 'otcuser'=>trim($mobile),'enname'=>$enname,'qz'=>$qz));
		$user_coin = array('userid' => $rs[0]);

		// 注册赠送币（直接赠送）
		/*if (C('give_type') == 1) {
			$coin_name = C('xnb_mr_song'); //赠送币种
			//$user_coin[$coin_name] = C('xnb_mr_song_num'); // 赠送注册用户币种

			// 赠送邀请人邀请奖励
			if(C('song_num_1') > 0 && $invit_1 > 0){

			    //必须通过初级认证
			    if(($invituser['kyc_lv'] == 1 && $invituser['idstate']==2) || $invituser['kyc_lv'] > 1 ){
                    ...
                    $user_coinInfo =  $this->getUserCoin($invit_1);
                    ...
                    $rs[] = $mo->table('tw_finance_log')->add(array('username' => $invituser['username'], 'adminname' => session('admin_username'), 'addtime' => time(), 'plusminus' => 1, 'amount' =>  $coin_num_1, 'optype' => 30, 'cointype' => 9, 'old_amount' => $user_coinInfo[$coin_name], 'new_amount' => $sum_money[$coin_name], 'userid' => $user_coin, 'adminid' => session('admin_id'),'addip'=>get_client_ip()));

			    }

                //$rs[] = crateRecord($invit_1,$coin_name,1,$address='',$remark='',$coin_num_1,$user_coinInfo[$coin_name]);
				$coin_num_1 = C('song_num_1');
				$rs[] = $mo->table('tw_invit')->fetchSql()->add(array('userid' => $invit_1, 'invit' => $rs[0], 'name' => '一代注册赠送', 'type' => '注册赠送'.strtoupper($coin_name), 'num' => 0, 'mum' => 0, 'fee' => $coin_num_1, 'addtime' => time(), 'status' => 0,'coin'=>strtoupper($coin_name)));
                $user_coinInfo =  $this->getUserCoin($invit_1);
                $sum_money[$coin_name] = $user_coinInfo[$coin_name] + $coin_num_1;
                $rs[] = $mo->table('tw_user_coin')->where(array('userid'=>$invit_1))->save($sum_money);
			...
			if(C('song_num_2') > 0 && $invit_2 > 0){
                ...
                $user_coinInfo =  $this->getUserCoin($invit_2);
                $sum_money[$coin_name] = $user_coinInfo[$coin_name] + $coin_num_2;
			...
			if(C('song_num_3') > 0 && $invit_3 > 0){
                ...
                $user_coinInfo =  $this->getUserCoin($invit_3);
                $sum_money[$coin_name] = $user_coinInfo[$coin_name] + $coin_num_3;
...
//		$rs[] = crateRecord($rs[0],$coin_name,4,$address='',$remark='前台注册用户',0,0);
...
//			$mo->execute('unlock tables');
	...
	public function getUserCoin($invit_id = NULL)
    {
        $mo = M();
        $where ['userid'] = $invit_id;
        $user_coinInfo =  $mo->table('tw_user_coin')->where($where)->find();
        return $user_coinInfo;
    }
	// 注册成功页面
	...
	public function findpwd()
	{
		if (IS_POST) {
			...
			if($user['password'] == user_md($input['password'])){
...
//			$mo->execute('lock tables tw_user write , tw_user_log write ');
			$rs = array();
			$rs[] = $mo->table('tw_user')->where(array('id' => $user['id']))->save(array('password' => user_md($input['password'])));
