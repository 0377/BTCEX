...
class MoneyController extends HomeController
{
	...
	public function index()
	{
		if (IS_POST) {
			...
			$rs[] = $mo->table('tw_money_log')->add(array('userid' => userid(), 'moneyid' => $money['id'], 'name' => $money['name'], 'num' => $num, 'tian' => $money['tian'], 'fee' => $money['fee'], 'feecoin' => $money['feecoin'], 'addtime' => time(), 'status' => 0));
		...
		foreach ($MoneyList as $money) {
			...
			if ($money['endtime'] < $money['lasttime']) {
				...
				if ($MoneyLogList) {
					...
					foreach ($MoneyLogList as $user_money_list) {
						...
						$rs[] = $mo->table('tw_money_log')->save(array('id' => $user_money_list['id'], 'status' => 0));
						$rs[] = $mo->table('tw_money_dlog')->add(array('userid' => $user_money_list['userid'], 'money_id' => $money['id'], 'type' => 1, 'num' => $user_money_list['num'], 'addtime' => time(), 'content' => '理财结束,退回本金:' . $user_money_list['num'] . '个'));
			...
			if (!$money['lasttime'] || (($money['lasttime'] + $money['step']) < time())) {
				...
				foreach ($MoneyLogList as $MoneyLog) {
					...
					$rs[] = $mo->table('tw_money_dlog')->add(array('userid' => $MoneyLog['userid'], 'money_id' => $money['id'], 'type' => 1, 'num' => $fee, 'addtime' => time(), 'content' => '本金:' . $money['coinname'] . ' :' . $MoneyLog['num'] . '个,获取理财利息' . $money['feecoin'] . ' ' . $fee . '个'));
