...
/*		if (!$user['idcard']) {
	...
	// 用户实名认证 - 提交初级认证
	public function kyc1_Handle($idnationality, $idtype, $truename, $idcard,$idimg1,$idimg2)
	{
		// 过滤非法字符----------------S
		if (checkstr($idnationality) || checkstr($idtype) || checkstr($truename) || checkstr($idcard) || checkstr($idimg1) || checkstr($idimg2)) {
...
/*			if (!check($truename, 'truename')) {
		...
		if (M('User')->where(array('id' => userid()))->save(array('kyc_lv' => 1, 'idnationality' => $idnationality, 'idtype' => $idtype, 'truename' => $truename, 'idcard' => $idcard, 'idstate' => 1,'idimg1' => $idimg1, 'idimg2' => $idimg2))) {
	...
	public function kyc2_Handle($idimg1, $idimg2, $idimg3)
	{
        ...
        if($user['idimg1'] && $user['idimg2']){
            if(!$idimg3){
                $this->error(L('请上传证件照后再提交！'));
            }
            if (M('User')->where(array('id' => userid()))->save(array('kyc_lv' => 2,'idimg3' => $idimg3, 'idstate' => 1))) {
	...
	public function uppassword($mobile_verify, $oldpassword, $newpassword, $repassword)
	{
		...
		if (user_md($oldpassword) != $password) {
			$this->error(L('旧登录密码错误！'));
		}
		if (md5($newpassword) == $paypasswords) {
			$this->error(L('登录密码不能和交易密码相同！'));
		}
		if (user_md($newpassword) == $password) {
			$this->error(L('新登录密码跟原密码相同，修改失败！'));
		}

		$rs = M('User')->where(array('id' => userid()))->save(array('password' => user_md($newpassword)));
		if ($rs) {
			$this->success(L('修改成功'));
		} else {
			$this->error(L('修改失败'));
	...
	public function uppaypasswordset($paypassword, $repaypassword, $mobile_verify)
	{
		...
		if (M('User')->where(array('id' => userid(), 'password' => user_md($paypassword)))->find()) {
...
/*		if ($user_info['mobile'] != session('chkmobile')) {
	...
	public function mobile()
	{
		...
		$this->assign('user', $user);
		$this->assign('prompt_text', D('Text')->get_content('user_mobile'));
		$this->display();
	}
	...
	public function uptpwdsetting($paypassword, $tpwdsetting)
	{
        ...
        $user_tpwdsetting = M('User')->where(array('id' => userid()))->getField('tpwdsetting');
        if ($tpwdsetting == $user_tpwdsetting) {
            $this->error(L('设置没有改变！'));
        }else{
            $rs = M('User')->where(array('id' => userid()))->save(array('tpwdsetting' => $tpwdsetting));
            if ($rs) {
                $userid = userid();
                session($userid. 'tpwdsetting',$tpwdsetting);
                $this->success(L('成功！'));
            } else {
                $this->error(L('失败！'));
            }
...
/*		if (!check($name, 'a')) {
		...
		if(empty($user_paypassword)){
	            $this->error(L('请设置交易密码！'),U('User/index'));
	        }
		if (md5($paypassword) != $user_paypassword) {
			$this->error(L('交易密码错误！'));
		}
