...
class ToolsController extends AdminController
{
	protected function _initialize(){
		parent::_initialize();
		$allow_action=array("index","cache","delcahe","getDirSize","queue","qianbao","delcache","invoke","rmdirr","excel","export","import","stats");
...
//				$mo->execute('unlock tables');
	...
	public function stats(){
		...
		$user_coin = M('user_coin');
		$turnout = D('myzc');
		$turnin = M('myzr');


		//已开放币种总额
		$btcCount= $user_coin->sum('btc');
		$ethCount= $user_coin->sum('eth');
		$hrcCount= $user_coin->sum('hrc');
		$usdtCount= $user_coin->sum('usdt');
		$astcCount= $user_coin->sum('astc');


		$biName= array('btc','eth','hrc','usdt','astc');
		$num= array($btcCount,$ethCount,$hrcCount,$usdtCount,$astcCount);
		$list = array_combine($biName,$num);
...
//		$where['addtime']=array('between',$data);
		$where['addtime']=array('between',array($data["star"],$data["end"]));

		$todayOut = $turnout->field("SUM(num) as sumout,coinname,addtime")->where($where)->group('coinname')->select();
		$todayIn = $turnin->field("SUM(mum) as sumin,coinname,addtime")->where($where)->group('coinname')->select();


		$arr = [];
		$out = [];
		$in = [];
		foreach($todayOut as $key=>$value){
			$out[$value['coinname']] = $value['sumout'];
		}
		foreach($todayIn as $key=>$value){
			$in[$value['coinname']] = $value['sumin'];
		}
		$max = $out>=$in?$out:$in; //取大值
		$min = $out<=$in?$out:$in; //取大值
		$sumax = $out>=$in?'sumout':'sumin'; //1为转入2为转出
		$sumin = $out>=$in?'sumin':'sumout'; //1为转入2为转出
		foreach($max as $key => $value){
			$arr[$key][$sumax] = isset($value)?$value:0;
			$arr[$key][$sumin] = isset($min[$key])?$min[$key]:0;
		}
