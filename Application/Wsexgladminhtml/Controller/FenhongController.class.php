...
//			if (!check($_POST['num'], 'd')) {
			...
			$builder->keyText('name', '分红名称', '可以中文');
...
//			$mo->execute('lock tables tw_user_coin write,tw_fenhong_log write');
			$rs = array();
            $info = queryUserCoin($fenfa[$id]['userid'],$fenhong['coinjian']);
			$rs[] = $mo->table('tw_user_coin')->where(array('userid' => $fenfa[$id]['userid']))->setInc($fenhong['coinjian'], $faduoshao);
			// user_coin 操作记录
            $rs[] = crateRecord($fenfa[$id]['userid'],$fenhong['coinjian'],$type=19,$address='',$remark='分红奖励',$coin_num=$faduoshao,$coin_old=$info[$fenhong['coinjian']]);
