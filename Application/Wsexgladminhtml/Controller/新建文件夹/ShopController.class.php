...
//		$mo->execute('lock tables tw_user_coin write,tw_shop_log write');
		$rs[] = $mo->table('tw_user_coin')->where(array('userid' => $shoplog['userid']))->setInc($shoplog['coinname'], $shoplog['xuyao']);
		$rs[] = $mo->table('tw_shop_log')->where(array('id' => $id))->save(array('status' => 2));
