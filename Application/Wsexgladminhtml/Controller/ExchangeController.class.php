...
class ExchangeController extends AdminController
{
	...
	{
	    $data = M('complant c')
            ->join('tw_user as u ON c.userid=u.id')
            ->field('u.enname,u.username,u.mobile,u.truename,u.idcard,c.id,c.img,c.content,c.status,c.create_time')
            ->select();
//	    dump($data);exit;
    ...
    //改变审核状态
    public function cancelComplant(){
        $res = M('complant')->where(['id'=>I('id')])->setField('status',2);
        if($res){
            $arr = [
                'msg'=>'成功',
                'status'=>1,
...
//        dump(M()->getLastSql());exit;
		...
		if ($status == 1 || $status == NULL) {
			$list = M('exchange_order')->where($where)->order('status asc,id desc')->limit($Page->firstRow . ',' . $Page->listRows)->select();
		} else {
		    
			$list = M('exchange_order')->where($where)->order('status desc,id asc')->limit($Page->firstRow . ',' . $Page->listRows)->select();
		}
//		dump(M()->getLastSql());exit;
		foreach ($list as $k => $v) {
			$aids = '';
			...
			$list[$k]['truename'] = M('user')->where(['id'=>$v['userid']])->getField('truename');
		}
    ...
    public function cancel(){
	    $id = I('id');
	    $res = M('exchange_order')->where(['id'=>$id])->setField('status',8);
	    if($res){
           $this -> success('操作成功');
        }else{
            $this -> error('操作失败');
...
//			$mo->execute('lock tables tw_user_coin write,tw_exchange_order write,tw_finance write,tw_finance_log write,tw_user read, tw_exchange_config read, tw_market read');
			...
			// 用户账户数据处理
			$rs[] = $mo->table('tw_user_coin')->where(array('userid' => $mycz['userid']))->setInc($types, $nums);
            $rs[] = crateRecord($mycz['userid'],$coinName=$types,18,$address='',$remark='C2C充值',$mums,'0.00');
			...
			if ($configs['give_type'] == 1 && $configs['xnb_mr_song_num'] > 0 && $nums >= $qbsong_num)
			{
				if (!(M('finance_log')->where(array('userid'=>$mycz['userid'],'description'=>array('like',"%首次充值赠送%")))->find())) {
					...
					if ($configs['grant_type'] == 1) {
                        ...
                        $rs[] = crateRecord($mycz['userid'],$coin_name,18,$address='',$remark='C2C首次充值赠送奖励',$user_coin_num,$finance_num_user_coin[$coin_name]);
						// 处理资金变更日志-----------------S
				...
				if ($configs['grant_type'] == 1) {
					...
					/* 直接发放奖励 */
					$rs[] = $mo->table('tw_user_coin')->where(array('userid' => $mycz['userid']))->setInc($coin_name, $user_coin_num);
                    $rs[] = crateRecord($mycz['userid'],$coin_name,18,$address='',$remark='C2C每次充值赠送奖励',$user_coin_num,$finance_num_user_coin[$coin_name]);
...
//				$mo->execute('unlock tables');
	...
	// C2C提现记录
	public function mytx($name = NULL, $status = NULL,$time_type = NULL)
	{
		$where = array();
		/* 用户名--条件 */
		if ($field && $name) {
		...
		// 默认状态
		if ($status == "") {
			$where['_string'] = '(status = 1 OR status = 2 OR status = 3 OR status = 8)';
		}
		/* 状态--条件 */
		if ($status != '99') {
			if ($status) {
...
//		dump($time_type);exit;
		...
		$Page = new \Think\Page($count, 15);
		$show = $Page->show();
//		echo M('exchange_order')->getLastSql();exit;
//		dump($count);exit;
...
//			$mo->execute('unlock tables');
	...
	// C2C代理商
	public function agent($field="",$name='',$status='',$time_type=NULL)
	{
		$where = array();
		/* 用户名--条件 */
		if ($field && $name) {
	...
	public function agentEdit($id = NULL)
	{
		if (empty($_POST)) {
			...
			if ($rs) {
				$this->success('编辑成功！',U('Exchange/agent'));
			} else {
				$this->error('编辑失败！');
			}
		}	
