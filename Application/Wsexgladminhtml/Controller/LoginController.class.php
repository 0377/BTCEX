...
//            }
            ...
            if ($admin['password'] !=  admin_md($password)) {
                $this->error('用户名或密码错误！');
            } else {
                $uids = $admin['id'];
			...
			/*if ($admin['password'] !=  md5($password)) {
				...
				$this->success('登陆成功!', U('Wsexgladminhtml/Index/index'));
			}
		} else {
			...
			if (session('admin_id')) {
				$this->redirect('Wsexgladminhtml/Index/index');
			}
	...
	public function unlock()
	{
		if (!session('admin_id')) {
			session(null);
			$this->error('登录已经失效,请重新登录...', '/Wsexgladminhtml/login');
		}
		if (session('LockScreenTime') < 0) {
			session(null);
			$this->error('密码错误过多,请重新登录...', '/Wsexgladminhtml/login');
		}

		$pass = trim(I('post.pass'));
		if ($pass == session('LockScreen')) {
			session('LockScreen', null);
			$this->success('解锁成功', '/Wsexgladminhtml/index');
		}

		$admin = M('Admin')->where(array('id' => session('admin_id')))->find();
		if ($admin['password'] == md5(md5($pass.ADMIN_SALT))) {
			session('LockScreen', null);
			$this->success('解锁成功', '/Wsexgladminhtml/index');
		}
