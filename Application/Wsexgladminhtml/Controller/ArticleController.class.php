	...
	public function edit($id = NULL, $type = NULL)
	{
		if (empty($_POST)) {
			...
			if ($type == 'images') {
				...
				if ($rs) {
					$this->success('编辑成功！', U('Wsexgladminhtml/Article/index'));
				} else {
					$this->error('编辑失败！');
				}
			}
...
//		$upload->rootPath = './Upload/wenzhang/';
	...
	public function typeEdit($id = NULL, $type = NULL)
	{
		...
		if (empty($_POST)) {
			...
			if ($type == 'images') {
				...
				if ($rs) {
					$this->success('编辑成功！',U('Article/type'));
				} else {
					$this->error('编辑失败！');
				}
			}
	...
	// 广告管理 新增&编辑
	public function adverEdit($id = NULL, $type = NULL)
    {
        if (empty($_POST)) {
            if ($id) {
...
//                    var_dump($rs);
	...
	public function youqingEdit($id = NULL)
	{
		if (empty($_POST)) {
			...
			if ($rs) {
				$this->success('编辑成功！',U('Article/youqing'));
			}
			else {
				$this->error('编辑失败！');
			}
